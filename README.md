...
![Version](https://img.shields.io/badge/version-4.3.1-2ea44f)
...
```text
  ════════════════════════════════════════════════════════════════════════════
  NETWORK-WIREGUARD-MANAGER                                             v4.3.1
...
Sur un **hôte Proxmox**, ce menu s'efface si pve-firewall est actif ; s'il est inactif, il propose de gérer le pare-feu de l'hôte ici même (voir [Proxmox](#-proxmox--hôte-et-vm)).
...
- **Sur un hôte Proxmox** : l'**optimisation réseau** s'adapte (profil `pve-host`, tuning de la carte physique sous `vmbr0`). Docker y est bloqué, WireGuard déconseillé (préfère une VM). Pour le **pare-feu**, le script applique la règle « jamais deux pare-feux » : si **pve-firewall est actif**, il s'efface et te renvoie vers l'interface web ; si **pve-firewall est inactif**, le menu 6 propose de **gérer le pare-feu de l'hôte directement** (accord explicite mémorisé) — avec ouverture guidée de l'interface web (8006/tcp) restreinte à tes IP admin, et retrait automatique du filtrage si pve-firewall est réactivé plus tard. Les VM bridgées ne passent pas par ce filtrage : leur sécurité se gère dans chaque VM. Sur un cluster multi-nœuds, reste plutôt sur pve-firewall (ou ouvre corosync `5405-5412/udp` d'abord).
